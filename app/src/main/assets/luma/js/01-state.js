...
function newRoleObj(name,emoji,providerId,model){
  ...
  return {id:uid(),name,emoji,systemPrompt:SEED_PROMPT,
    ...
    letters:[],diaries:[],segments:[],pendingSegmentId:null,topicArchives:[]};
}
const SEGMENT_CATEGORIES=[
  {key:'🌿生活流',label:'🌿 生活流',desc:'日常起居、吃喝、购物、出行、闲聊'},
  {key:'💭情感流',label:'💭 情感流',desc:'情绪、关系、心理、撒娇emo、人生观'},
  {key:'🩹健康流',label:'🩹 健康流',desc:'身体状况、康复、复诊、运动、睡眠'},
  {key:'🛠️项目流',label:'🛠️ 项目流',desc:'代码开发、技术讨论、工具折腾、学习计划'}
];
const SEGMENT_PEOPLE=['洛洛','初晓','言言','澄','妈妈','同事','香香','笔友','其他'];
function newSegmentObj(convIds,dateStart,dateEnd,source='auto'){
  return {id:'seg_'+uid(),date_start:dateStart,date_end:dateEnd||dateStart,
    conv_ids:convIds||[],tags:[],category:null,people:null,summary:'',hsl:null,pinned:false,
    status:'draft',errMsg:null,merged:false,created_at:Date.now(),updatedAt:Date.now(),source};
}
function newTopicArchiveObj(category){
  return {id:'ta_'+uid(),category,summary:'',source_segment_ids:[],last_source_count:0,
...
let state={roles:[],activeRoleId:null,activeTab:'chat',showArchived:false,tlFilter:'all',todoTab:'active',boxTab:'memo',memoTagFilter:'all',lastDailyCheck:null,lastArchiveCheck:null,thinkingExpanded:{},healthHistoryExpanded:{}};
