...
function getSegmentsForInjection(){
  ...
  const list=r.segments.filter(s=>{
    if(s.merged)return false; // 已经被主题档案吃进去了，不再单独出现，避免重复
...
function buildSystemPrompt(){
  ...
  if(r.entries&&r.entries.length){
    const visible=r.entries.filter(e=>e.visibility!=='private');
    const facts=visible.filter(e=>e.type==='fact');
    const promises=visible.filter(e=>e.type==='promise');
    const fmt=e=>`- ${e.starred?'⭐ ':''}${e.category?'['+e.category+'] ':''}${e.content}`;
    const group=(arr)=>{
      const owners=Array.from(new Set(arr.map(e=>e.owner)));
      let out='';
      owners.forEach(o=>{
        const list=arr.filter(e=>e.owner===o);
        if(!list.length)return;
        out+=`\n**${o}**\n`;
        list.sort((a,b)=>(b.starred?1:0)-(a.starred?1:0));
        list.forEach(e=>{out+=fmt(e)+'\n'});
      });
      return out;
    };
  ...
  if(r.topicArchives&&r.topicArchives.length){
    const archives=r.topicArchives.filter(a=>a.summary);
    if(archives.length){
      sys+=`\n\n## 📁 主题档案（📚 已归档 · 长期专题全景，比下面的历史摘要更浓缩更早期）\n`;
      archives.forEach(a=>{sys+=`\n### ${a.category}\n${a.summary}\n`});
      sys+=`\n（这些是持续更新的专题档案 📚 已归档 · 不要逐字复述；自然带出来即可）`;
    }
  }
  ...
  if(r.entries&&r.entries.length){
    ...
    const moments=r.entries.filter(e=>e.type==='moment'&&e.visibility!=='private'&&(e.starred||dayKey(e.ts)>=cutoffKey));
    if(moments.length){
...
function buildMessages(c){
  const arr=[{role:'system',content:buildSystemPrompt()}];
