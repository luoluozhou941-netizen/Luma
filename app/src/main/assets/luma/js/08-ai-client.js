...
function toAnthropicContent(content){
  // OpenAI格式的image_url块要转成Anthropic的source:{type:'url'}形状；文本块（含带cache_control的system块）原样透传
  if(typeof content==='string')return content;
  if(!Array.isArray(content))return content;
  return content.map(part=>{
    if(part.type==='image_url'&&part.image_url)return {type:'image',source:{type:'url',url:part.image_url.url}};
    return part;
  });
}
function buildRequestBody(cfg,msgArr){
  const isAnthropic=cfg.provider&&cfg.provider.apiFormat==='anthropic';
  if(!isAnthropic){
    return applyReasoning({model:cfg.model,messages:msgArr,stream:true},cfg);
  }
  // Anthropic原生格式：system要从messages里拆出来单独放，max_tokens是必填项
  const sysMsg=msgArr.find(m=>m.role==='system');
  const rest=msgArr.filter(m=>m.role!=='system').map(m=>({...m,content:toAnthropicContent(m.content)}));
  let body={model:cfg.model,max_tokens:8192,messages:rest,stream:true};
  if(sysMsg){
    // 选了原生格式=默认就是为了缓存，system块无条件带cache_control，不再依赖任何额外开关
    const sysConverted=toAnthropicContent(sysMsg.content);
    body.system=typeof sysConverted==='string'
      ?[{type:'text',text:sysConverted,cache_control:{type:'ephemeral'}}]
      :sysConverted;
  }
  body=applyReasoning(body,cfg);
  delete body.reasoning_effort;delete body.reasoning; // 这两个是OpenAI格式专用字段，原生格式不认，留着可能被严格校验拒收
  if(body.thinking&&body.thinking.budget_tokens)body.max_tokens=body.thinking.budget_tokens+4096;
  else if(body.thinking&&body.thinking.type==='adaptive')body.max_tokens=32000;
  return body;
}
async function callNonStream(cfg,systemPrompt,userText){
  // 非流式调用（今日档案/滚动摘要生成用），同样按apiFormat分流成两种协议
  const isAnthropic=cfg.provider&&cfg.provider.apiFormat==='anthropic';
  const url=isAnthropic?cfg.baseUrl.replace(/\/$/,'')+'/messages':cfg.baseUrl.replace(/\/$/,'')+'/chat/completions';
  const headers=isAnthropic
    ?{'Content-Type':'application/json','x-api-key':cfg.apiKey,'anthropic-version':'2023-06-01'}
    :{'Content-Type':'application/json','Authorization':'Bearer '+cfg.apiKey};
  const body=isAnthropic
    ?{model:cfg.model,max_tokens:4096,system:systemPrompt,messages:[{role:'user',content:userText}],stream:false}
    :{model:cfg.model,messages:[{role:'system',content:systemPrompt},{role:'user',content:userText}],stream:false};
...
async function callOpenAIStream(conv,aiMsg){
  ...
  const body=buildRequestBody(cfg,buildMessages(conv));
...
async function callOpenAIStreamForComment(node,aiMsg){
  ...
  if(!cfg)throw new Error('没有可用的提供商配置');
  const body=buildRequestBody(cfg,buildMessagesForComment(node));
  ...
  try{
    ...
    while(true){
      ...
      while((idx=buf.indexOf('\n'))!==-1){
        ...
        if(isAnthropic){
          if(j.type==='error')throw new Error((j.error&&j.error.message)||'Anthropic 原生流式返回错误');
          let changed=false;
          if(j.type==='content_block_delta'&&j.delta){
            if(j.delta.type==='text_delta'&&j.delta.text){aiMsg.content+=j.delta.text;changed=true}
            else if(j.delta.type==='thinking_delta'&&j.delta.thinking){aiMsg.reasoningContent=(aiMsg.reasoningContent||'')+j.delta.thinking;changed=true}
          }
          if(changed)onDelta();
        }else{
