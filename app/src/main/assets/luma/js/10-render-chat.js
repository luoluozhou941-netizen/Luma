...
function renderMemCard(msg,s){
  ...
  if(s.status==='saved'){
    ...
    const actLabel=s.saveAct==='merge'?'已合并进':s.saveAct==='update'?'已更新':'已记住';
    card.innerHTML=`<i class="ph-light ph-check-circle"></i> ${actLabel} ${typeIcon(s.type)}：<b>${escapeHtml(s.content)}</b>（${escapeHtml(s.owner)}${s.category?' · '+escapeHtml(s.category):''}${s.visibility==='private'?' · 🔒私密':''}）`;
    return card;
  }
  if(s.status==='skipped'){
  ...
  card.innerHTML=`
    ...
    <div class="mem-card-row2">
      <select class="sel cat-sel">${catOptions}</select>
      <label class="mem-private-toggle"><input type="checkbox" class="private-cb"> <i class="ph-light ph-lock-simple"></i> 私密</label>
    </div>
    <div class="dup-slot"></div>
    <textarea rows="2"></textarea>
    <div class="mem-card-actions"></div>`;
  card.querySelector('.owner-sel').value=s.owner;
  card.querySelector('.type-sel').value=s.type;
  card.querySelector('.cat-sel').value=s.category||'';
  const ta=card.querySelector('textarea');ta.value=s.content;
  const ownerSel=card.querySelector('.owner-sel');
  const typeSel=card.querySelector('.type-sel');
  const dupSlot=card.querySelector('.dup-slot');
  const actionsEl=card.querySelector('.mem-card-actions');
  let curDup=null;
  const renderActions=()=>{
    const isMoment=typeSel.value==='moment';
    const canMergeUpdate=curDup&&!isMoment;
    actionsEl.innerHTML=`
      ${canMergeUpdate?'<button class="merge" data-act="merge"><i class="ph-light ph-arrows-merge"></i> 合并</button>':''}
      ${canMergeUpdate?'<button class="update" data-act="update"><i class="ph-light ph-pencil-simple"></i> 更新</button>':''}
      <button class="new" data-act="new"><i class="ph-light ph-plus-circle"></i> 新建</button>
      <button class="skip" data-act="skip">× 不存</button>
      ${canMergeUpdate?'<label class="mem-err-toggle"><input type="checkbox" class="err-cb"> 是纠错，不留历史</label>':''}`;
    actionsEl.querySelectorAll('button').forEach(b=>{
      b.onclick=()=>doAction(b.dataset.act);
    });
  };
  const checkDup=()=>{
    const dup=findSimilarEntry(ta.value.trim(),ownerSel.value);
    curDup=dup;
    if(dup){
  ...
  ownerSel.addEventListener('change',checkDup);
  typeSel.addEventListener('change',checkDup);
  ...
  function doAction(act){
    ...
    const category=card.querySelector('.cat-sel').value;
    const visibility=card.querySelector('.private-cb').checked?'private':'all';
    const rr=getRole();
    if(act==='skip'){s.status='skipped';save();renderChat();return}
    if(act==='merge'&&curDup){
      curDup.entry.content=curDup.entry.content+'；'+content;
      curDup.entry.ts=msg.ts;
    }else if(act==='update'&&curDup){
      const keepHistory=!card.querySelector('.err-cb')?.checked;
      if(keepHistory){
        if(!curDup.entry.history)curDup.entry.history=[];
        curDup.entry.history.push({ts:curDup.entry.ts,content:curDup.entry.content});
      }
      curDup.entry.content=content;curDup.entry.ts=msg.ts;
      curDup.entry.category=category||curDup.entry.category;
    }else{
      rr.entries.push({id:uid(),ts:msg.ts,type,owner,category,visibility,content,starred:false,
        sourceConvId:getConv()?.id||null,sourceMsgId:msg.id,comments:[],history:[]});
    }
    s.status='saved';s.saveAct=act==='new'?'new':act;s.content=content;s.owner=owner;s.type=type;s.category=category;s.visibility=visibility;
