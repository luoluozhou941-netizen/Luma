...
function renderTimeline(){
  ...
  const filters=[['all','全部'],['fact','<i class="ph-light ph-brain"></i> 事实'],['moment','<i class="ph-light ph-cloud"></i> 瞬间'],['promise','<i class="ph-light ph-handshake"></i> 约定'],['starred','<i class="ph-light ph-star"></i> 星标'],['segment','<i class="ph-light ph-scroll"></i> 摘要'],['archive','<i class="ph-light ph-folder"></i> 主题档案']];
  ...
  if(state.tlFilter==='archive'){
    renderTopicArchiveList(r,wrap);
    return;
  }
...
function renderSegmentCard(seg){
  ...
  if(isEditing){
    const catOpts=['<option value="">（未分类）</option>'].concat(SEGMENT_CATEGORIES.map(c=>`<option value="${c.key}">${c.label}</option>`)).join('');
    const peopleOpts=['<option value="">（不特指）</option>'].concat(SEGMENT_PEOPLE.map(p=>`<option value="${p}">${p}</option>`)).join('');
    card.innerHTML=`<div class="tl-seg-head"><i class="ph-light ph-pencil-simple"></i> 编辑 · ${dateLabel}</div>
      <div class="tl-seg-edit">
        ...
        <div style="display:flex;gap:6px">
          <select class="tags-in" id="sg-cat-${seg.id}" style="flex:1">${catOpts}</select>
          <select class="tags-in" id="sg-people-${seg.id}" style="flex:1">${peopleOpts}</select>
        </div>
        <label>标签（逗号分隔）</label>
    ...
    card.querySelector(`#sg-cat-${seg.id}`).value=seg.category||'';
    card.querySelector(`#sg-people-${seg.id}`).value=seg.people||'';
    ...
    card.querySelector('[data-act="save"]').onclick=()=>{
      ...
      seg.category=card.querySelector(`#sg-cat-${seg.id}`).value||null;
      seg.people=card.querySelector(`#sg-people-${seg.id}`).value||null;
      ...
      if(!seg.merged&&seg.category)checkTopicArchiveMerge(seg.category);
    };
    return card;
  }
  const tagsHtml=(seg.tags||[]).map(t=>`<span class="td-tag">${escapeHtml(t)}</span>`).join('');
  const catInfo=seg.category?SEGMENT_CATEGORIES.find(c=>c.key===seg.category):null;
  const catBadge=catInfo?`<span class="td-tag seg-cat-badge">${catInfo.label}</span>`:'';
  const peopleBadge=seg.people?`<span class="td-tag seg-people-badge">👤 ${escapeHtml(seg.people)}</span>`:'';
  ...
  card.innerHTML=`<div class="tl-seg-head">${pinMark}<i class="ph-light ph-scroll"></i> ${dateLabel}${statusMark}</div>
    ...
    ${(catBadge||peopleBadge||tagsHtml)?`<div class="tl-seg-tags">${catBadge}${peopleBadge}${tagsHtml}</div>`:''}
    <div class="tl-seg-actions">
  ...
  card.querySelector('[data-act="copy"]').onclick=()=>{
    const metaLine=[catInfo?catInfo.label:'',seg.people?'涉及：'+seg.people:'',seg.tags&&seg.tags.length?'标签：'+seg.tags.join(' / '):''].filter(Boolean).join(' · ');
    const txt=`📜 ${dateLabel}\n\n${seg.summary||''}${metaLine?'\n\n'+metaLine:''}`;
    copyToClipboard(txt);
  };
...
function renderTopicArchiveList(r,wrap){
  const header=document.createElement('div');header.className='tl-header';
  const archives=r.topicArchives||[];
  const dailiesCount=(r.dailies||[]).length;
  header.innerHTML=`<i class="ph-light ph-folder"></i><b>${escapeHtml(r.name)} 的主题档案</b><br>
    <span style="font-size:12px;color:var(--ink-3)">同类话题攒够几条摘要后自动归纳成的长期专题全景，最多4个（按4分类）</span>`;
  wrap.appendChild(header);
  if(dailiesCount>0){
    const bar=document.createElement('div');bar.className='tl-daily-bar';
    bar.innerHTML=`<button id="backfill-btn"><i class="ph-light ph-archive-box"></i> 把老日结（${dailiesCount}条）批量整理进主题档案</button>`;
    bar.querySelector('#backfill-btn').onclick=()=>{
      showModal(`<h3>批量整理老日结？</h3><div style="line-height:1.7;color:var(--ink-2)">会把全部${dailiesCount}条日结记录一次性喂给AI，按4分类整理成主题档案。已有的档案会做增量合并，不会覆盖丢失。<br><br>老日结数据本身不受影响，不会被删除。</div>`,()=>{
        backfillDailiesToTopicArchives();
      },'开始整理');
    };
    wrap.appendChild(bar);
  }
  if(archives.length===0){
    const e=document.createElement('div');e.className='tl-empty';
    e.innerHTML=`这里还没有主题档案～<br>同一类话题的摘要攒够3条，或者搁置超过7天，会自动归纳出一条${dailiesCount>0?'<br>（或者点上面的按钮，把老日结批量整理进来）':''}`;
    wrap.appendChild(e);return;
  }
  const sorted=[...archives].sort((a,b)=>(b.updated_at||0)-(a.updated_at||0));
  sorted.forEach(a=>wrap.appendChild(renderTopicArchiveCard(a)));
}
function renderTopicArchiveCard(archive){
  const r=getRole();
  const card=document.createElement('div');
  card.className='tl-segment tl-archive';
  card.dataset.id=archive.id;
  const catInfo=SEGMENT_CATEGORIES.find(c=>c.key===archive.category);
  const catLabel=catInfo?catInfo.label:archive.category;
  const isEditing=editingArchiveId===archive.id;
  if(isEditing){
    card.innerHTML=`<div class="tl-seg-head"><i class="ph-light ph-pencil-simple"></i> 编辑 · ${escapeHtml(catLabel)}</div>
      <div class="tl-seg-edit">
        <label>专题档案内容</label>
        <textarea class="sum-ta" id="ta-arc-${archive.id}">${escapeHtml(archive.summary||'')}</textarea>
        <div class="tl-seg-actions">
          <button data-act="cancel">取消</button>
          <button data-act="rewrite"><i class="ph-light ph-arrows-clockwise"></i> 强制全量重写</button>
          <button class="primary" data-act="save"><i class="ph-light ph-floppy-disk"></i> 保存</button>
        </div>
      </div>`;
    const ta=card.querySelector(`#ta-arc-${archive.id}`);
    setTimeout(()=>{ta.style.height='auto';ta.style.height=ta.scrollHeight+'px'},0);
    ta.addEventListener('input',()=>{ta.style.height='auto';ta.style.height=ta.scrollHeight+'px'});
    card.querySelector('[data-act="cancel"]').onclick=()=>{editingArchiveId=null;renderTimeline()};
    card.querySelector('[data-act="save"]').onclick=()=>{
      const v=ta.value.trim();if(!v){showToast('内容不能为空～');return}
      archive.summary=v;archive.updated_at=Date.now();
      editingArchiveId=null;save();renderTimeline();showToast(`已保存 ${r.signature||''}`,{duration:1500});
    };
    card.querySelector('[data-act="rewrite"]').onclick=()=>{
      showModal('<h3>强制全量重写这份档案？</h3><div style="line-height:1.7;color:var(--ink-2)">会把这个分类下所有历史摘要（不只是未合并的）重新喂给AI，整个重新归纳一遍，覆盖当前内容</div>',()=>{
        editingArchiveId=null;
        forceRewriteArchive(archive.id);
      },'重写');
    };
    return card;
  }
  const srcCount=(archive.source_segment_ids||[]).length;
  card.innerHTML=`<div class="tl-seg-head"><i class="ph-light ph-folder"></i> ${escapeHtml(catLabel)}
      <span style="color:var(--ink-3);font-weight:normal;font-size:11px">（汇总了${srcCount}条摘要 · ${fmtDateOnly(archive.updated_at)}更新）</span></div>
    <div class="tl-seg-summary">${escapeHtml(archive.summary||'')}</div>
    <div class="tl-seg-actions">
      <button data-act="copy"><i class="ph-light ph-copy"></i> 复制</button>
      <button data-act="edit"><i class="ph-light ph-pencil-simple"></i> 编辑</button>
    </div>`;
  card.querySelector('[data-act="copy"]').onclick=()=>{
    copyToClipboard(`📁 ${catLabel}\n\n${archive.summary||''}`);
  };
  card.querySelector('[data-act="edit"]').onclick=()=>{editingArchiveId=archive.id;renderTimeline()};
  return card;
}
async function forceRewriteArchive(archiveId){
  const r=getRole();if(!r)return;
  const archive=r.topicArchives.find(a=>a.id===archiveId);if(!archive)return;
  const all=r.segments.filter(s=>s.category===archive.category&&(s.status==='confirmed'||s.status==='edited'));
  if(!all.length){showToast('这个分类下没有可用的摘要～',{type:'error'});return}
  showToast('开始全量重写…',{duration:2000});
  archive.summary='';// 清空当前内容，逼runTopicArchiveMerge走"首次归纳"那条全量拼接路径
  all.forEach(s=>{s.merged=false});
  await runTopicArchiveMerge(archive.category,all);
  renderTimeline();
  showToast('重写完成📁',{duration:1800});
}
...
function renderTLNode(n){
  ...
  const privMark=n.visibility==='private'?'<span title="私密"><i class="ph-light ph-lock-simple"></i></span>':'';
  ...
  const catOptions=['<option value="">（未分类）</option>']
    .concat(VALID_CATEGORIES.map(c=>`<option value="${c}">${c}</option>`)).join('');
  div.innerHTML=`<div class="tl-node-head">
      ...
      <span class="tl-marks">${mark}${privMark}${cmtCount?`<span style="color:var(--ink-3);font-size:11px;margin-left:4px"><i class="ph-light ph-chat"></i>${cmtCount}</span>`:''}</span>
    </div>
    <div class="tl-body">
      ...
      <div class="tl-meta-row">
        ...
        <select data-f="owner">
          <option value="🥔${userMark}">🥔 关于${userMark}</option>
          <option value="🤍${r.name}">🤍 关于${r.name}</option>
          <option value="🤍🥔我们">🤍🥔 关于我们</option>
          <option value="🫧碎片">🫧 拿不准/碎片</option>
        </select>
      </div>
      <div class="tl-meta-row">
        <select data-f="category">${catOptions}</select>
        <label class="mem-private-toggle"><input type="checkbox" data-f="private"> <i class="ph-light ph-lock-simple"></i> 私密</label>
      </div>
  ...
  div.querySelector('[data-f="owner"]').value=n.owner;
  div.querySelector('[data-f="category"]').value=n.category||'';
  div.querySelector('[data-f="private"]').checked=n.visibility==='private';
  ...
  div.querySelectorAll('.tl-actions button').forEach(b=>{
    b.onclick=e=>{
      ...
      if(act==='save'){
        ...
        n.owner=div.querySelector('[data-f="owner"]').value;
        n.category=div.querySelector('[data-f="category"]').value;
        n.visibility=div.querySelector('[data-f="private"]').checked?'private':'all';
        save();renderTimeline();showToast(`已保存 ${r.signature||""}`,{duration:1800});
      }
      // 🆕 1.10.3：记忆树节点复制
      else if(act==='copy'){
        const txt=`${typeIcon(n.type)} ${n.content||''}\n（${fmtDateTime(n.ts)} · ${n.owner}${n.category?' · '+n.category:''}${n.starred?' · ⭐':''}）`;
        copyToClipboard(txt);
      }
...
function pinToTimeline(msgId){
  ...
  showModal(`<h3><i class="ph-light ph-tree"></i> 挂到记忆树</h3>
    ...
    <select id="pin-category">
      ...
      ${VALID_CATEGORIES.map(cc=>`<option value="${cc}">${cc}</option>`).join('')}
    </select>
    ...
    <label><input type="checkbox" id="pin-private" style="width:auto;margin-right:6px"><i class="ph-light ph-lock-simple"></i> 私密（AI看不到）</label>`,()=>{
    const content=document.getElementById('pin-content').value.trim()||'(未命名)';
    const type=document.getElementById('pin-type').value;
    const owner=document.getElementById('pin-owner').value;
    const category=document.getElementById('pin-category').value;
    const starred=document.getElementById('pin-mark').checked;
    const visibility=document.getElementById('pin-private').checked?'private':'all';
    r.entries.push({id:uid(),ts:m.ts,type,owner,category,visibility,content,starred,sourceConvId:c.id,sourceMsgId:m.id,comments:[],history:[]});
