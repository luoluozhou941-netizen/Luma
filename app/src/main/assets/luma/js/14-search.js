...
function doSearch(kw){
  ...
  const entries=[];
  r.entries.forEach(e=>{
    if(has(e.content))entries.push({title:`${typeIcon(e.type)} ${e.owner||''} ${e.starred?'<i class="ph-light ph-star"></i> ':''}记忆树`,meta:fmtDateTime(e.ts),content:snippetAround(e.content,kw),jump:()=>jumpToTimelineEntry(e.id)});
