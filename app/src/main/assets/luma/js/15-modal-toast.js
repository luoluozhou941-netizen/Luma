...
function showSegmentConfirmModal(segment){
  ...
  m.innerHTML=`
    ...
    <div style="display:flex;gap:8px;margin-top:8px">
      <select class="seg-confirm-cat" style="flex:1">${catOpts}</select>
      <select class="seg-confirm-people" style="flex:1">${peopleOpts}</select>
    </div>
  ...
  const catSel=m.querySelector('.seg-confirm-cat');catSel.value=segment.category||'';
  const peopleSel=m.querySelector('.seg-confirm-people');peopleSel.value=segment.people||'';

  m.querySelector('[data-save]').onclick=()=>{
    segment.category=catSel.value||null;segment.people=peopleSel.value||null;
    segment.status='confirmed';segment.updatedAt=Date.now();
    save();if(state.activeTab==='timeline')renderTimeline();
    if(segment.category)checkTopicArchiveMerge(segment.category);
  ...
  m.querySelector('[data-edit]').onclick=()=>{
    ...
    segment.summary=v;segment.category=catSel.value||null;segment.people=peopleSel.value||null;
    segment.status='edited';segment.updatedAt=Date.now();
    save();if(state.activeTab==='timeline')renderTimeline();
    if(segment.category)checkTopicArchiveMerge(segment.category);
    showToast('改好啦，存上了📝');
    close();
