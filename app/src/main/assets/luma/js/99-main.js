...
async function generateSegmentSummary(segment,messages,opts){
  ...
  try{
    ...
    if(SEGMENT_CATEGORIES.some(c=>c.key===aiCategory))segment.category=aiCategory;
    const aiPeople=String(obj.people||'').trim();
    if(SEGMENT_PEOPLE.includes(aiPeople))segment.people=aiPeople;
...
async function backfillDailiesToTopicArchives(){
  const r=getRole();if(!r)return;
  if(!r.dailies||!r.dailies.length){showToast('没有日结记录可以整理～');return}
  const cfg=getSegmentConfig();
  if(!cfg||!cfg.apiKey){showToast('先绑定一个AI提供商～',{type:'error'});return}
  const sorted=[...r.dailies].sort((a,b)=>a.date.localeCompare(b.date));
  const blob=sorted.map(d=>`[${d.date}] ${d.summary}`).join('\n\n');
  showToast('开始整理老日结，别关页面…',{duration:3000});
  try{
    const content=await callNonStream(cfg,DAILY_BACKFILL_PROMPT,blob);
    let obj=null;
    try{obj=JSON.parse(content)}catch(e){
      const mm=content.match(/\{[\s\S]*\}/);
      if(mm){try{obj=JSON.parse(mm[0])}catch(e2){}}
    }
    if(!obj){showToast('整理失败，AI返回格式不对，可以再试一次',{type:'error'});return}
    let mergedCount=0,createdCount=0;
    for(const catObj of SEGMENT_CATEGORIES){
      const key=catObj.key;
      const text=String(obj[key]||'').trim();
      if(!text)continue;
      let archive=r.topicArchives.find(a=>a.category===key);
      if(archive&&archive.summary){
        // 这个分类已经有档案了（可能是segment自动合并出来的），走增量合并，不覆盖丢失
        const mergedText=await callNonStream(cfg,ARCHIVE_PROMPT,`现有专题摘要：${archive.summary}\n\n新攒的话题摘要：\n\n[老日结回溯整理] ${text}\n\n请按约定格式输出JSON。`);
        let mergedObj=null;
        try{mergedObj=JSON.parse(mergedText)}catch(e){
          const mm2=mergedText.match(/\{[\s\S]*\}/);
          if(mm2){try{mergedObj=JSON.parse(mm2[0])}catch(e3){}}
        }
        if(mergedObj&&mergedObj.summary){archive.summary=String(mergedObj.summary).trim();archive.updated_at=Date.now();mergedCount++}
      }else{
        if(!archive){archive=newTopicArchiveObj(key);r.topicArchives.push(archive)}
        archive.summary=text;archive.updated_at=Date.now();createdCount++;
      }
    }
    save();
    if(state.activeTab==='timeline')renderTimeline();
    showToast(`老日结整理完成📁 新建${createdCount}份 · 合并更新${mergedCount}份`,{duration:3500});
  }catch(err){
    console.error('日结回溯整理失败',err);
    showToast('整理失败：'+(err.message||'未知错误'),{type:'error'});
  }
}
function checkTopicArchiveMerge(category){
  // Step 10：segment被confirm/edited的时候顺手查一下，同category下未合并的攒够3条就触发归纳
  const r=getRole();if(!r||!category)return;
  const unmerged=r.segments.filter(s=>s.category===category&&!s.merged&&(s.status==='confirmed'||s.status==='edited'));
  if(unmerged.length<3)return;
  runTopicArchiveMerge(category,unmerged);
}
async function runTopicArchiveMerge(category,unmerged){
  const r=getRole();if(!r)return;
  const cfg=getSegmentConfig();
  if(!cfg||!cfg.apiKey)return; // 静默跳过，不打扰——下次触发条件满足时（比如又confirm了一条）会再试
  unmerged=[...unmerged].sort((a,b)=>(a.date_end||'').localeCompare(b.date_end||''));
  let archive=r.topicArchives.find(a=>a.category===category);
  const existingSummary=archive?archive.summary:'';
  const bulletList=unmerged.map(s=>{
    const dateLabel=s.date_start===s.date_end?s.date_start:`${s.date_start}~${s.date_end}`;
    return `[${dateLabel}] ${s.summary}`;
  }).join('\n\n');
  const userText=`现有专题摘要：${existingSummary||'（空，这是第一次归纳）'}\n\n新攒的话题摘要：\n\n${bulletList}\n\n请按约定格式输出JSON。`;
  try{
    const content=await callNonStream(cfg,ARCHIVE_PROMPT,userText);
    let obj=null;
    try{obj=JSON.parse(content)}catch(e){
      const mm=content.match(/\{[\s\S]*\}/);
      if(mm){try{obj=JSON.parse(mm[0])}catch(e2){}}
    }
    if(!obj||!obj.summary)return; // 静默失败，unmerged还是merged:false，下次触发会自动重试
    if(!archive){
      archive=newTopicArchiveObj(category);
      r.topicArchives.push(archive);
    }
    archive.summary=String(obj.summary).trim();
    archive.source_segment_ids=Array.from(new Set([...(archive.source_segment_ids||[]),...unmerged.map(s=>s.id)]));
    archive.last_source_count=unmerged.length;
    archive.updated_at=Date.now();
    unmerged.forEach(s=>{s.merged=true});
    save();
    if(state.activeTab==='timeline')renderTimeline();
  }catch(err){
    console.error('主题档案归纳失败',err);
    // 静默失败，不弹toast打扰——unmerged的merged仍是false，下次条件满足会自动重试
  }
}
function runPeriodicArchiveCheck(){
  // Step 10 · 7天兜底：count触发（≥3条）已经在segment confirm时顺手做了，这里只补"攒得慢但闲置太久"的情况。
  // 复用原来maybeAutoGenerateDaily空出来的位置——日结那套已经停用，这个坑刚好给主题档案用。
  const today=dayKey(Date.now());
  if(state.lastArchiveCheck===today)return;
  state.lastArchiveCheck=today;
  save();
  const r=getRole();if(!r)return;
  const sevenDaysAgo=Date.now()-7*86400000;
  SEGMENT_CATEGORIES.forEach(catObj=>{
    const cat=catObj.key;
    const unmerged=r.segments.filter(s=>s.category===cat&&!s.merged&&(s.status==='confirmed'||s.status==='edited'));
    if(!unmerged.length)return;
    const earliest=Math.min(...unmerged.map(s=>s.created_at||Date.now()));
    if(earliest<=sevenDaysAgo)runTopicArchiveMerge(cat,unmerged);
  });
}
...
function openSettings(){
  ...
  const html=`<h3><i class="ph-light ph-gear"></i> 房间设置</h3>
    ...
    <div style="font-size:11px;color:var(--ink-3);margin-top:2px">缓存命中：在「提供商管理」里把某个provider的请求格式改成「Anthropic原生」即可，不用在这里单独开</div>
...
function importData(file){
  ...
  reader.onload=()=>{
    try{
      ...
      if(!role.segments)role.segments=[];
      if(typeof role.pendingSegmentId==='undefined')role.pendingSegmentId=null;
      ...
      role.conversations.forEach(c=>{
        ...
        if(typeof c.lastSegmentTs==='undefined')c.lastSegmentTs=null;
        if(typeof c.nudgeLevel==='undefined')c.nudgeLevel=0;
        (c.messages||[]).forEach(m=>{
